...
public class PerformanceTestService {
    ...
    public void delete(DeleteTestPlanRequest request) {
        ...
        List<TestResource> resourceList = testResourceService.getResourcesByLoadTestId(testId);

        if (resourceList.size()>0) {
            for (TestResource testResource : resourceList) {
                testResource.setLoadTestId("");
    ...
    private LoadTestWithBLOBs saveLoadTest(SaveTestPlanRequest request) {
        ...
        loadTest.setTestResourcePoolSlaveNum(request.getTestResourcePoolSlaveNum());
    ...
    private void startEngine(LoadTestWithBLOBs loadTest, Engine engine, String triggerMode) {
        ...
        try {
            ...
            List<TestResource> resourceList = engine.getResourceList();
            for (TestResource testResource:resourceList) {
                testResource.setLoadTestId(loadTest.getId());
                testResourceService.updateTestResource(testResource);
            }
    ...
    public List<DashboardTestDTO> dashboardTests(String organizationId) {
        Instant oneYearAgo = Instant.now().plus(-365, ChronoUnit.DAYS);
        long startTimestamp = oneYearAgo.toEpochMilli();
        return extLoadTestReportMapper.selectDashboardTests(organizationId, startTimestamp);
    }
    ...
    private void stopEngine(String reportId) {
        ...
        List<TestResource> resourceList = testResourceService.getFreeResourcesByPoolId(loadTest.getTestResourcePoolId());
        if (org.apache.commons.collections.CollectionUtils.isEmpty(resourceList)) {
