...
public class JmeterDocumentParser implements DocumentParser {
    ...
    public String parse(EngineContext context, Document document) throws Exception {
        ...
        for (int i = 0; i < childNodes.getLength(); i++) {
            ...
            if (node instanceof Element) {
                ...
                parseHashTree(context, ele);

                processFileExceptCSV(ele);
            }
        }
    ...
    private void parseHashTree(EngineContext context, Element hashTree) {
        if (invalid(hashTree)) {
            return;
        }
            ...
            for (int i = 0; i < childNodes.getLength(); i++) {
                ...
                if (node instanceof Element) {
                    ...
                    if (nodeNameEquals(ele, HASH_TREE_ELEMENT)) {
                        ...
                        if (context.getProperty("loopCountEnable") == null) {
                            processThreadGroup(ele);
                            processConcurrencyThreadGroup(ele);
                            processCheckoutTimer(ele);
                            processCheckoutBackendListener(ele);
                        } else {
                            Object loopCountEnables = context.getProperty("loopCountEnable");
                            if (loopCountEnables instanceof List) {
                                Object o = ((List<?>) loopCountEnables).get(0);
                                ((List<?>) loopCountEnables).remove(0);
                                if (o == null || "false".equals(o.toString())) {
                                    ...
                                    Object loopCounts = context.getProperty("loopCount");
                                    String loopCount;
                                    if (loopCounts instanceof List) {
                                        Object loopCountObject = ((List<?>) loopCounts).get(0);
                                        ((List<?>) loopCounts).remove(0);
                                        loopCount = loopCountObject.toString();
                                    } else {
                                        loopCount = loopCounts.toString();
                                    }
                                    processConcurrencyThreadGroup(ele);
                                    processLoopControllerInThreadGroup(ele,loopCount);
                                    processCheckoutTimer(ele);
                                    processCheckoutBackendListener(ele);
    ...
    private void processFileExceptCSV(Element hashTree) {
        if (invalid(hashTree)) {
            return;
        }
        String type = hashTree.getAttribute("elementType");
        String name = hashTree.getAttribute("name");
        String testname = hashTree.getAttribute("testname");
        LogUtil.debug("into 递归 type:" + type + ",name:" + name + ",testname:" + testname);
        ...
        if (hashTree.getChildNodes().getLength() == 0) {
            ...
            for (int i = 0; i < childNodes.getLength(); i++) {
                ...
                if (node instanceof Element) {
                    ...
                    LogUtil.debug("into loop type:" + type + ",name:" + name + ",testname:" + testname);
                    if (nodeNameEquals(ele, ELEMENT_PROP)) {
                        String elementTypeTag = ele.getAttribute("elementType");
                        if (StringUtils.equals(elementTypeTag, "HTTPFileArg")) {
                            ...
                            LogUtil.debug("yyyfilename1:" + filename);
                            filename = filename.substring(filename.lastIndexOf(separator) + 1);
                            LogUtil.debug("yyyfilename2:" + filename);
                            ele.setAttribute("name", "/test/" + filename);
                        }
                    } else if (nodeNameEquals(ele, STRING_PROP)) {
                        String filepathTag = ele.getAttribute("name");
                        if (StringUtils.equals(filepathTag, "File.path")) {
                            // 截取文件名
                            String separator = "/";
                            String filename = ele.getTextContent();
                            if (!StringUtils.contains(filename, "/")) {
                                separator = "\\";
                            }
                            LogUtil.debug("zzzfilename1:" + filename);
                            filename = filename.substring(filename.lastIndexOf(separator) + 1);
                            LogUtil.debug("zzzfilename2:" + filename);
                            ele.setTextContent("/test/" + filename);
                        }
                    }
                    processFileExceptCSV(ele);
        ...
        for (int i = 0; i < childNodes.getLength(); i++) {
            ...
            if (item instanceof Element && nodeNameEquals(item, STRING_PROP)) {
                ...
                if (StringUtils.equals(filenameTag, "filename")) {
                    ...
                    LogUtil.debug("qqqfilename1:" + filename);
                    filename = filename.substring(filename.lastIndexOf(separator) + 1);
                    LogUtil.debug("qqqfilename2:" + filename);
...
//        threadGroup.appendChild(createStringProp(document, "Iterations", "1"));
    ...
    private void processLoopControllerInThreadGroup(Element threadGroup,String loopCount) {
        String testname = threadGroup.getAttribute("testname");
        threadGroup.setAttribute("testname", testname + "-" + context.getResourceIndex());
        String type = threadGroup.getAttribute("elementType");
        String name = threadGroup.getAttribute("name");

        LogUtil.debug("into 递归 type:" + type + ",name:" + name + ",testname:" + testname);

        if (threadGroup.getChildNodes().getLength() == 0) {
            return;
        } else {
            final NodeList childNodes = threadGroup.getChildNodes();
            for (int i = 0; i < childNodes.getLength(); i++) {
                Node node = childNodes.item(i);
                if (node instanceof Element) {
                    Element ele = (Element) node;
                    ...
                    if (nodeNameEquals(ele, ELEMENT_PROP)) {
                        String elementTypeTag = ele.getAttribute("elementType");
                        if (StringUtils.equals(elementTypeTag, "LoopController")) {
                            processLoopCount(ele, loopCount);
                        }

                    } else if (nodeNameEquals(ele, BOOL_PROP)) {
                        String boolPropName = ele.getAttribute("name");
                        if (StringUtils.equals(boolPropName, "ThreadGroup.scheduler")) {
                            ele.setTextContent("false");
                        }
                    } else if (nodeNameEquals(ele, STRING_PROP)) {
                        String boolPropName = ele.getAttribute("name");
                        if (StringUtils.equals(boolPropName, "ThreadGroup.num_threads")) {
                            ...
                            if (targetLevels instanceof List) {
                                ...
                                targetLevel = o.toString();
                            } else {
                                targetLevel = targetLevels.toString();
                            }
                            ele.setTextContent(targetLevel);
                        }
                    }
                }
            }
    ...
    private void processLoopCount(Element threadGroup,String loopCount) {
            ...
            for (int i = 0; i < childNodes.getLength(); i++) {
                ...
                if (node instanceof Element) {
                    ...
                    if (nodeNameEquals(ele, STRING_PROP)) {
                        String elementTypeTag = ele.getAttribute("elementType");
                        if (StringUtils.equals(elementTypeTag, "LoopController.loops")) {
                            ele.setTextContent(loopCount);
                        }

                    } else if (nodeNameEquals(ele, BOOL_PROP)) {
                        String boolPropName = ele.getAttribute("name");
                        if (StringUtils.equals(boolPropName, "LoopController.continue_forever")) {
                            ele.setTextContent("false");
                        }
                    }
                    processFileExceptCSV(ele);
