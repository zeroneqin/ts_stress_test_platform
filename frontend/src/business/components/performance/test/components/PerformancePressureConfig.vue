    ...
    <el-row>
      <el-col>
        <el-form :inline="true">
          ...
          <el-form-item :label="$t('load_test.select_resource_pool_slave_num')">
            <el-input-number
              :disabled="isReadOnly"
              v-model="resourcePoolSlaveNum"
              @change="checkSlaveNum(resourcePoolSlaveNum)"
              :min="0"
              size="mini"/>
          </el-form-item>
            ...
            <el-form :inline="true">
              <el-form-item :label="$t('load_test.thread_num')">
                <el-input-number
                  ...
                  :min="resourcePoolSlaveNum"
                  size="mini"/>
              </el-form-item>
              <br>
              <el-form-item :label="$t('load_test.duration')">
                <el-switch v-model="threadGroup.durationEnable" @change="setEnable4Duration(threadGroup)"/>
                <el-input-number
                  :disabled="isReadOnly || !threadGroup.durationEnable"
              ...
              <br>
              <el-form-item :label="$t('load_test.loop_count')">
                <el-switch v-model="threadGroup.loopCountEnable" @change="setEnable4LoopCount(threadGroup)"/>
                <el-input-number
                  :disabled="isReadOnly || !threadGroup.loopCountEnable"
                  v-model="threadGroup.loopCount"
                  :min="1"
                  @change="calculateChart(threadGroup)"
                  size="mini"/>
              </el-form-item>
              ...
              <br>
              <!-- qinjun comment step raleted config
              <el-form-item :label="$t('load_test.ramp_up_time_within')">
                <el-input-number
                  :disabled="isReadOnly || !threadGroup.rampUpTimeEnable"
                  :min="1"
              ...
              <el-form-item :label="$t('load_test.ramp_up_time_minutes')">
                <el-input-number
                  :disabled="isReadOnly || !threadGroup.stepEnable"
                  :min="1"
...
const DURATION_ENABLE = "durationEnable";
const LOOP_COUNT_ENABLE = "loopCountEnable";
const LOOP_COUNT = "loopCount";
const RAMP_UP_TIME_ENABLE="rampUpTimeEnable";
const STEP_ENABLE="stepEnable";
const HOLD = "Hold";
...
export default {
  ...
  data() {
    return {
      ...
      durationEnable: true,
      loopCountEnable: false,
      rampUpTimeEnable: true,
      stepEnable: true,
      options: {},
      resourcePool: null,
      resourcePoolSlaveNum: 0,
      resourcePools: [],
      activeNames: ["0"],
      threadGroups: [],
  ...
  mounted() {
    ...
    this.resourcePoolSlaveNum = this.test.testResourcePoolSlaveNum;
    this.getResourcePools();
  },
  watch: {
    test(n) {
      this.resourcePool = n.testResourcePoolId;
      this.resourcePoolSlaveNum = n.testResourcePoolSlaveNum;
    },
    testId() {
      if (this.testId) {
        this.getJmxContent();
  ...
  methods: {
            ...
            if (d instanceof Array) {
              d.forEach(item => {
                switch (item.key) {
                  ...
                  case RPS_LIMIT_ENABLE:
                    ...
                    break;
                  case DURATION_ENABLE:
                    this.threadGroups[i].durationEnable = item.value;
                    break;
                  case LOOP_COUNT_ENABLE:
                    this.threadGroups[i].loopCountEnable = item.value;
                    break;
                  case LOOP_COUNT:
                    this.threadGroups[i].loopCount = item.value;
                    break;
                  case RAMP_UP_TIME_ENABLE:
                    this.threadGroups[i].rampUpTimeEnable = item.value;
                    break;
                  case STEP_ENABLE:
                    this.threadGroups[i].stepEnable = item.value;
                    break;
                  default:
                    break;
                }
              ...
              switch (d.key) {
                ...
                case RPS_LIMIT_ENABLE:
                  ...
                  break;
                case DURATION_ENABLE:
                  this.threadGroups[0].durationEnable = d.value;
                  break;
                case LOOP_COUNT_ENABLE:
                  this.threadGroups[0].loopCountEnable = d.value;
                  break;
                case LOOP_COUNT:
                  this.threadGroups[0].loopCount = d.value;
                  break;
                case RAMP_UP_TIME_ENABLE:
                  this.threadGroups[0].rampUpTimeEnable = d.value;
                  break;
                case STEP_ENABLE:
                  this.threadGroups[0].stepEnable = d.value;
                  break;
                default:
                  break;
              }
    ...
    checkSlaveNum(slaveNum) {
      ...
      if (this.resourcePoolResourceLength<slaveNum) {
        this.$warning(this.$t('load_test.resource_pool_size_less_than_slave_num'));
    ...
    setEnable4Duration(threadGroup) {
      if (threadGroup.durationEnable) {
        threadGroup.loopCountEnable=false;
        threadGroup.rampUpTimeEnable=true;
        threadGroup.stepEnable=true;
      }
      else {
        threadGroup.loopCountEnable=true;
        threadGroup.rampUpTimeEnable=false;
        threadGroup.stepEnable=false;
      }
    },
    setEnable4LoopCount(threadGroup) {
      if (threadGroup.loopCountEnable) {
        threadGroup.durationEnable=false;
        threadGroup.rampUpTimeEnable=false;
        threadGroup.stepEnable=false;
      }
      else {
        threadGroup.durationEnable=true;
        threadGroup.rampUpTimeEnable=true;
        threadGroup.stepEnable=true;
      }
    },
    ...
    validConfig() {
      ...
      for (let i = 0; i < this.threadGroups.length; i++) {
        ...
        if (this.threadGroups[i].threadNumber % this.resourcePoolSlaveNum !=0) {
          this.$warning(this.$t('load_test.pressure_config_threadnum_invalid_with_slavenum'));
          this.$emit('changeActive', '1');
          return false;
        }

        if (this.threadGroups[i].rpsLimitEnable && (this.threadGroups[i].rpsLimit%this.resourcePoolSlaveNum !=0)) {
          this.$warning(this.$t('load_test.pressure_config_rpslimit_invalid_with_slavenum'));
          this.$emit('changeActive', '1');
          return false;
        }
      }
    ...
    convertProperty() {
      ...
      for (let i = 0; i < this.threadGroups.length; i++) {
        result.push([
          ...
          {key: DURATION_ENABLE, value: this.threadGroups[i].durationEnable},
          {key: LOOP_COUNT_ENABLE, value: this.threadGroups[i].loopCountEnable},
          {key: LOOP_COUNT, value: this.threadGroups[i].loopCount},
          {key: RAMP_UP_TIME_ENABLE, value: this.threadGroups[i].rampUpTimeEnable},
          {key: STEP_ENABLE, value: this.threadGroups[i].stepEnable},
